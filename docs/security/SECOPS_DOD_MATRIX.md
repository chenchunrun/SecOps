...
- [ ] Manual TUI acceptance pass is recorded separately as release closeout.
- [ ] Packaging/output verification is recorded separately as release closeout.
...
- Full regression gate:
  - `CGO_ENABLED=0 GOCACHE=$(pwd)/.gocache go test ./... -count=1` (latest run passed)
...
## Release-Closeout Separation

The DoD items above are the code and regression baseline for SecOps delivery.
The remaining release-closeout items are tracked separately because they are
manual signoff activities rather than missing implementation:
...
- Windows packaging/output verification:
...
- Overall post-release checklist:
